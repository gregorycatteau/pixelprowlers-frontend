...
onMounted(() => {
  ...
  menuTl = gsap.timeline({ paused: true })
    .fromTo(
      menuContainer.value as HTMLElement,
      { x: '-100vw', opacity: 0 },
      { x: 0, opacity: 1, duration: 1, ease: 'power2.out' }
    )
    .fromTo(
      imageContainer.value as HTMLElement,
      { y: 50, opacity: 0, scale: 0.95 },
      { y: 0, opacity: 1, scale: 1, duration: 1, ease: 'power2.out' },
      '-=0.8'
    )
    .fromTo(
      navItems.value.filter(Boolean) as HTMLElement[],
      { y: 20, opacity: 0 },
...
watch(
  ...
  async (val) => {
    ...
    if (val) {
      ...
      await nextTick()
      menuTl?.play(0)
      idleAnim = gsap.to(imageContainer.value as HTMLElement, {
        y: 2,
        duration: 4,
        yoyo: true,
        repeat: -1,
...
const hoverClose = () => {
  if (prefersReducedMotion.value) return
  gsap.to(closeBtn.value as HTMLElement, {
    rotation: 20,
    yoyo: true,
    repeat: 1,
    duration: 0.3
...
const leaveClose = () => {
  if (prefersReducedMotion.value) return
  gsap.to(closeBtn.value as HTMLElement, { rotation: 0, duration: 0.3 })
}
